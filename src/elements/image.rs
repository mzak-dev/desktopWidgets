...
fn build(a: &mut Attrs) -> Result<Kind, String> {
    ...
    let ((w, h), ready) = a.request_image(&id);
    ...
    // `fade = 400`: a new `src` crossfades in over the old picture
    let fade = a.num("fade")?.unwrap_or(0.0).max(0.0).round() as u32;
    Ok(Kind::Image(ImageSpec { id, w, h, tint: a.color("tint")?, play, frame, fit, feather, fade, ready }))
}
